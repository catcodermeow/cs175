...
static shared_ptr<SgRbtNode> g_currentPickedEyeNode;
...
static void initSphere() {
    ...
    g_sphere.reset(new Geometry(&vtx[0], &idx[0], (int)vtx.size(), (int)idx.size()));
}
...
static ManipMode getManipMode() {
    if (g_currentPickedRbtNode == g_currentPickedEyeNode) {
        if (g_currentPickedEyeNode == g_skyNode && g_activeCameraFrame == WORLD_SKY)
...
static RigTForm getArcballRbt() {
    ...
    case ARCBALL_ON_SKY:
        return RigTForm();
    case EGO_MOTION:
        return getPathAccumRbt(g_world, g_currentPickedEyeNode);
    default:
...
static void updateArcballScale() {
    ...
    if (!g_currentPickedEyeNode)
        return;
        
    
    RigTForm arcballEye = inv(g_currentPickedEyeNode -> getRbt()) * getArcballRbt();
...
static void drawStuff(const ShaderState& curSS, bool picking) {
    ...
    const RigTForm eyeRbt = getPathAccumRbt(g_world, g_currentPickedEyeNode);
    ...
    if (!picking) {
        ...
        if (g_currentPickedRbtNode == g_groundNode || g_currentPickedRbtNode == NULL) {
            g_currentPickedRbtNode = g_currentPickedEyeNode; // choose current eye if you didn't pick an object
        }
...
static RigTForm moveArcball(const Cvec2 &p0, const Cvec2 &p1) {
    ...
    const RigTForm eyeInverse = inv(getPathAccumRbt(g_world, g_currentPickedEyeNode));
...
static void motion(GLFWwindow *window, double x, double y) {
    if (!g_mouseClickDown)
        return;
    if (g_currentPickedRbtNode == g_skyNode && g_currentPickedEyeNode != g_skyNode)
        return;
    ...
    RigTForm A = makeMixedFrame(getArcballRbt(), getPathAccumRbt(g_world, g_currentPickedEyeNode));

//    // the matrix for the auxiliary frame (the w.r.t.)
//    if(g_currentPickedEyeNode == NULL)
//
//    {
//        g_currentPickedEyeNode = g_skyNode;
//    }
    
    
    static shared_ptr<SgRbtNode> target;
...
static void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        ...
        case GLFW_KEY_F:
            ...
            break;
        case GLFW_KEY_V:
            g_eyeNodeindex = ((g_eyeNodeindex + 1) % 3);
                switch (g_eyeNodeindex) {
                    case 0:
                        g_currentPickedEyeNode = g_skyNode;
                        break;
                    case 1:
                        g_currentPickedEyeNode = g_robot1Node;
                        break;
                    case 2:
                        g_currentPickedEyeNode = g_robot2Node;
                        break;
                }
                
            cerr << "Active eye is " <<g_eyeNodeindex<< endl;
            break;
//        case GLFW_KEY_O:
...
struct JointDesc {
  ...
  ShapeDesc shapeDesc[NUM_SHAPES] = {
    ...
    {2,  ARM_LEN/2, 0, 0, ARM_LEN, ARM_THICK * 0.5f, ARM_THICK, g_cube}, // lower right arm
    {3, -ARM_LEN/2, 0, 0, ARM_LEN, ARM_THICK, ARM_THICK, g_sphere}, // upper left arm
    {4, -ARM_LEN/2, 0, 0, ARM_LEN, ARM_THICK * 0.5f, ARM_THICK, g_cube}, // lower left arm

    {5, 0,  HEAD_RADIUS, 0, HEAD_RADIUS, HEAD_RADIUS, HEAD_RADIUS, g_sphere}, // head

    {6, 0, -ARM_LEN/2, 0, ARM_THICK, ARM_LEN, ARM_THICK, g_sphere}, // upper right leg
    {7, 0, -ARM_LEN/2, 0, ARM_THICK * 0.5f, ARM_LEN, ARM_THICK, g_cube}, // lower right leg
    {8, 0, -ARM_LEN/2, 0, ARM_THICK, ARM_LEN, ARM_THICK, g_sphere}, // upper left leg
    {9, 0, -ARM_LEN/2, 0, ARM_THICK * 0.5f, ARM_LEN, ARM_THICK, g_cube}, // lower left leg
  };
